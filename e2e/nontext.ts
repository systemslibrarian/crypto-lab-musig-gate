 ...
 * its own, `minimumControlBoundaryRatio`, and it queried exactly
 * `.mono-input, .msg-input` — which is exactly the set the palette's
 ...
 * every BUTTON on this page took its edge from `--border`, a SURFACE divider,
 * and nothing ever measured it. It also compared the border against the
 * control's own FILL and never against the panel outside it, so a control could
 * pass while dissolving into the page.
 *
 ...
 *  - SVG shapes. The curve plot's key markers, accumulation path and aggregate
 *    marker are `<circle>`/`<polyline>`, and the legend keys are empty
 *    `<span class="cp-swatch">`s. None owns a text node and none is a control,
 *    so neither oracle judges them.
 *  - The two drawn `<select>` chevrons (`.styled-select`, `.wagner-select`),
 *    which are `background-image` gradient triangles on the control rather than
 *    a border, an outline or a pseudo-element.
 */
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * genuinely large: the BIP-327 vectors panel renders 56 `<details>` cases
     * in seven groups, and the curve plot is 126 `<circle>` siblings inside one
     * `<g>`. Without the caches the pass re-reads the same computed styles and
     * rects tens of thousands of times. Nothing mutates the DOM during the
     ...
     * theme. It is load-bearing on THIS page for a different reason: the two
     * styled `<select>`s carry `background-image` gradient chevrons, so reading
     * `background-color` alone would miss part of what their own box paints.
     */
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // A border only delineates if it PAINTS. `.tab-btn` ships
      // `border: 1px solid transparent` purely to reserve the 1px the active
      // state fills in, and an unselected tab therefore draws nothing at all —
      // it is identified by its text, which is the case the "is it trying to
      // draw itself as a control?" test below already exempts. Counting a
      // zero-alpha border as a boundary would instead report every such control
      // at a fixed 1.00:1 against a surround it is not trying to stand out
      // from, which is noise rather than a finding.
