 ...
 *    an unknown backdrop, or a `color-mix()`. This lab has three live ones:
 *    the shared top bar's `--cl-ink` is
 *    `color-mix(in srgb, var(--accent) 60%, #eafff8)` and paints every word in
 *    the bar; `.cell-moving` is
 *    `color-mix(in oklab, var(--accent) 10%, transparent)`, the tint that marks
 *    which cells a control just moved; and `.cl-hero-why` is authored as a
 *    `color-mix(in oklab, var(--accent) 6%, transparent)` before a later rule
 *    replaces it with an opaque `--accent-tint`. axe files the first two under
 *    `incomplete`, so a violations-only gate measured neither.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. In the plot that is
 *    `.cp-all circle` at `.5` and `.cp-kick` at `.75`; in the hero,
 *    `.cl-hero-sub` is authored at `.85` and then explicitly restored to `1`
 *    with a real colour, which is the shape this walk is here to keep honest —
 *    the moment the restoring rule is deleted the ratio moves and nothing else
 ...
 *    boxes are every `.table-wrap` (`overflow-x: auto`, which `scrollRegion()`
 *    wraps the coefficient, probe and vector tables in) and `.seg`
 *    (`overflow: hidden` around the two byte-mode buttons). Content scrolled
 ...
 *    load-bearing here rather than defensive: `curvePlot.ts` draws a real SVG
 *    figure, and its numbered key markers are a `<circle class="cp-keys">`
 *    with a `<text class="cp-num">` appended straight after it. The digit's ink
 *    is `--accent-ink`, a near-black that only makes sense over the circle's
 *    `--accent` fill; an ancestor-only walk would composite it onto the
 *    `<svg>`'s `--surface-2` panel instead and report a ratio nothing on screen
 *    has. The same applies to the `Q` marker (`.cp-agg-label` over
 *    `.cp-agg`). The stroke-only `<line>`s of `.cp-axes` and `.cp-kick` are
 *    excluded by the fill-shape test below, which matters because SVG's initial
 ...
 * rule. `nontext.ts` measures those separately, from the same paint model.
 *
 * SECOND, SVG shapes. A `<circle>`, `<polyline>` or CSS swatch carries no text
 * node, so nothing here judges it. That is WCAG 1.4.11 territory rather than
 * 1.4.3, it has no automated oracle at all, and for this lab it was measured by
 * hand — see the header of `nontext.ts` and the gate's own notes on the plot.
 */
 ...
 * uses. This lab's only short-lived styling is `.tour-flash`, a 1.4s ring the
 * tour paints on whatever it just jumped to — and under the reduced motion this
 * gate asserts, the stylesheet cancels it (`animation: none`), so the class can
 * be present without anything transient being on screen.
 *
 * The parameter is kept because the narrowed form is what makes a short-lived
 * state measurable at all: a class that lives one second is shorter than a full
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks five exhibits, and the expensive states are
     * genuinely large: the BIP-327 vectors panel renders 56 `<details>` cases in
     * seven groups, and the curve plot is 126 `<circle>` siblings inside one
     * `<g>`. Both are hundreds of elements re-walking the same ancestors up to
     ...
     * that is every `.table-wrap` (`overflow-x: auto`, which `scrollRegion()`
     * puts round the coefficient, probe and vector tables) and `.seg`
     * (`overflow: hidden` around the two byte-mode buttons).
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. BOTH branches are live on this page. `curvePlot.ts`
      // appends `<text class="cp-num">` immediately after the `<circle>` it
      // numbers, and `<text class="cp-agg-label">` immediately after the `Q`
      // marker — near-black `--accent-ink` digits that only read against those
      // fills, and which an ancestor-only walk would measure against the plot's
      // `--surface-2` panel instead. And `.cp-axes` builds its two rules from
      // stroke-only `<line>`s that cross both tick labels, which is precisely
      // the shape the FILLED test exists to keep out of the backdrop.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. This lab is full of
      // them: `.disclose`, `.learner-check`, `.glossary`, `#tour-advanced` and
      // the 56 `.kat-item` vector cases all ship shut. The gate opens each by
      ...
      // `position:absolute; width:1px; height:1px; clip-path: inset(50%)` — the
      // modern spelling, which `clippedToNothing` handles alongside the classic
      // `clip: rect(0,0,0,0)`. Two things use it, and both are live during the
      // drive: the tour's `role="status" aria-live="polite"` region, whose text
      // is rewritten at every one of its ten stops, and the ` done` word
      // `markTourProgress()` appends beside each finished tab's tick so the
      // state is not carried by colour and a glyph alone. Both have a rect and
      // pass `checkVisibility()` while painting exactly zero pixels; measuring
      // them reports ratios for ink that was never laid down, and the tour
      // region would report a fresh one on every scan. It is deliberately
     ...
     *
     * What this lab hides is a closed set and every member of it is a glyph
     * duplicating an adjacent word: the `✓`/`✕`/`⚠` that opens a `.verdict`,
     * a `.pill`, a `.stage` or a learner-check result; the `→` arrows in the
     * collapse diagram and the `.dep-chain`; the `.tour-dots` progress strip,
     * whose position is also printed as "Step N of 10"; and the `.cp-swatch`
     * colour keys in the plot legend, which sit immediately before the words
     * they key. In every case the meaning-bearing words sit beside the glyph in
     * the same ink on the same surface, so measuring the words measures the
     * glyph too — and the `.verdict-*` rules paint `.verdict-icon` and
     * `.verdict-word` from the same single declaration, which is what makes
     * that literally true here rather than merely likely.
     *
     * Nothing on this page hides a VALUE. No key, nonce, coefficient, partial
     * signature, verdict or vector result is inside an `aria-hidden` subtree —
     ...
     *
     * The plot is built from real `<g>` groups — `.cp-all`, `.cp-axes`,
     * `.cp-keys`, `.cp-path` — so this is the guard that keeps a stray text
     * node inside one of them out of the walk. `curvePlot.ts` builds its nodes
     * with `createElementNS` and appends strings explicitly, so nothing
     * currently lands text where it cannot render; the guard is what stops a
     * future `${array}` interpolation (which JS joins with commas, leaving a run
     * of "," text nodes) from being reported as a 1:1 failure for ink that was
     * never laid down.
     */
     ...
     * `style.css` sets `html { background-color: var(--bg) }` as a LONGHAND, so
     ...
     * whose dark palette bottoms out at `#100e0c`.
     */
